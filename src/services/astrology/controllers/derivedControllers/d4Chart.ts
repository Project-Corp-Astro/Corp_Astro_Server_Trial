...
// // // // export async function fetchD4Chart(user: any): Promise<ChartResponse> {
// // // //     const chartType = await ChartType.findOne({ where: { chart_code: 'D-4' } });
// // // //     if (!chartType) throw new Error('Chart type D-4 not found');
// // // //     const apiEndpoint = chartType.getDataValue('api_endpoint');
// // // //     if (!apiEndpoint) throw new Error('API endpoint not configured for D-4');
...
// // // //     return { chart_type: 'D-4', data: derivedData };
// // // //   }

  
// // // // // Main controller
// // // // export const fetchD4ChartData = async (req: Request, res: Response): Promise<void> => {
// // // //     try {
// // // //       const { id } = req.params;
...
// // // //       // Fetch all charts
// // // //       const chartResults = await fetchD4Chart(user);
  
// // // //       res.status(200).json({
// // // //         message: 'D4 chart data fetched successfully',
// // // //         data: chartResults,
// // // //       });
// // // //     } catch (error: any) {
...
// // //   let ascendantSign = derivedData.ascendant?.sign || '';
...
// // // // Helper function to fetch D-4 chart
// // // export async function fetchD4Chart(user: any): Promise<ChartResponse> {
// // //   const chartType = await ChartType.findOne({ where: { chart_code: 'D-4' } });
// // //   if (!chartType) throw new Error('Chart type D-4 not found');
// // //   const apiEndpoint = chartType.getDataValue('api_endpoint');
...
// // //     console.log('DerivedData for D-4:', JSON.stringify(derivedData, null, 2));
...
// // //     });
// // //     if (!existingChart) {
// // //       throw new Error('Failed to retrieve newly saved D-4 chart');
// // //     }
// // //   }
...
// // //     });
// // //   } catch (error: any) {
// // //     console.error('Error in fetchD4ChartData:', error);
// // //     if (error instanceof ValidationError) {
// // //       res.status(400).json({
// // //         message: 'Validation error occurred',
// // //         errors: error.errors.map((e: any) => ({
...
// // //       });
// // //     } else {
// // //       res.status(500).json({ message: 'Failed to fetch D-4 chart data', error: error.message });
// // //     }
// // //   }
// // // };
...
// //   console.log('DerivedData for D-4:', JSON.stringify(response.data, null, 2));
...
// //   const chartData = {
// //     ascendant: derivedData.d4_ascendant || derivedData.ascendant || {},
// //     planetary_positions: derivedData.planetary_positions || {},
// //     house_signs: derivedData.house_signs || [],
// //     aspects: derivedData.aspects || {},
// //     ayanamsa_value: derivedData.notes?.ayanamsa || derivedData.metadata?.ayanamsa || 'Lahiri',
// //   };
// //   const ascendantSign = derivedData.d4_ascendant?.sign || derivedData.ascendant?.sign || '';
// //   const metadata = {
...
// //           ascendant_sign: ascendantSign,
// //           ascendant_degree: parseDMSToDecimal(derivedData.d4_ascendant?.degrees || derivedData.ascendant?.degrees),
// //           ascendant_degree_formatted: derivedData.d4_ascendant?.degrees || derivedData.ascendant?.degrees || null,
...
// // // Update D-4 chart
// // export const updateD4Chart = async (
// //   user: User,
// //   chartType: any,
// //   apiEndpoint: string,
// //   existingChart: any
...
// // // Insert D-4 chart
// // export const insertD4Chart = async (req: Request, res: Response): Promise<void> => {
// //   try {
// //     const { id } = req.params;
...
// //     });
// //     if (existingChart) {
// //       res.status(400).json({ message: 'D-4 chart already exists for user' });
// //       return;
// //     }
...
// //     res.status(200).json({
// //       message: 'D4 chart inserted successfully',
// //       data: { derived_chart_id: derivedChartId, chart_type: 'D-4', data: derivedData },
// //     });
// //   } catch (error) {
// //     handleError(res, error, 'Failed to insert D-4 chart');
// //   }
// // };

// // // Fetch D-4 chart
// // export const fetchD4Chart = async (req: Request, res: Response): Promise<void> => {
// //   try {
// //     const { id } = req.params;
...
// //       where: { user_id: user.user_id, chart_type_id: chartType.chart_type_id },
// //     });
// //     let message = 'D4 chart fetched successfully';

// //     if (!existingChart) {
// //       const derivedData = await callAstroApi(apiEndpoint, buildApiPayload(user));
// //       const derivedChartId = await saveOrUpdateChartData(derivedData, chartType, user, apiEndpoint);
// //       existingChart = await DerivedChart.findOne({ where: { derived_chart_id: derivedChartId } });
// //       message = 'D4 chart created and fetched successfully';
// //     } else if (hasUserDetailsChanged(user, existingChart.calculation_parameters)) {
// //       const derivedChartId = await updateD4Chart(user, chartType, apiEndpoint, existingChart);
// //       existingChart = await DerivedChart.findOne({ where: { derived_chart_id: derivedChartId } });
// //       message = 'D4 chart updated and fetched successfully';
// //     }

// //     if (!existingChart) throw new Error('Failed to retrieve D-4 chart');
...
// //     });
// //   } catch (error) {
// //     handleError(res, error, 'Failed to fetch D-4 chart');
// //   }
// // };
...
//   if (cached) {
//     console.log('Returning D4 chart from Redis cache.');
...
//     const cacheKey = `d4chart:${user.user_id}`;
...
//     const redisKey = `d4_chart_user_${updatedUser.user_id}`;
//     const cacheKey = `d4chart:${updatedUser.user_id}`;
...
//     ) {
//       console.log(`Serving D-4 chart for userId ${updatedUser.user_id} from Redis cache`);
//       res.status(200).json({
//         message: 'D4 chart fetched successfully from cache',
...
export const fetchD4Chart = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!existingChart) {
      ...
      const derivedChartId = await updateD4Chart(updatedUser, chartType, apiEndpoint, existingChart);
      existingChart = await DerivedChart.findOne({ where: { derived_chart_id: derivedChartId } });
      message = 'D4 chart updated and fetched successfully';
    }
