...
// // //   const chartData = {
// // //     ascendant: derivedData.surya_lagna || {},
...
// // //   const ascendantSign = derivedData.surya_lagna?.sign || '';
...
// // //     ascendant_degree: derivedData.surya_lagna?.longitude && typeof derivedData.surya_lagna.longitude === 'string'
// // //       ? parseFloat(derivedData.surya_lagna.longitude.split('°')[0])
// // //       : 0,
// // //     ascendant_degree_formatted: derivedData.surya_lagna?.longitude || null,
...
// // // // Helper function to fetch Sun Chart
// // // export async function fetchSunChart(user: any): Promise<ChartResponse> {
// // //   const chartType = await ChartType.findOne({ where: { chart_code: 'SUN' } });
// // //   if (!chartType) throw new Error('Chart type SUN not found');
// // //   const apiEndpoint = chartType.getDataValue('api_endpoint');
// // //   if (!apiEndpoint) throw new Error('API endpoint not configured for Sun Chart');
...
// // //     console.log('DerivedData for Sun Chart:', JSON.stringify(derivedData, null, 2));
...
// // //     });
// // //     if (!existingChart) {
// // //       throw new Error('Failed to retrieve newly saved Sun Chart');
// // //     }
// // //   }
...
// // // // Main controller
// // // export const fetchSunChartData = async (req: Request, res: Response): Promise<void> => {
// // //   try {
// // //     const { id } = req.params;
...
// // //     // Fetch Sun Chart
// // //     const chartResults = await fetchSunChart(user);
...
// // //     });
// // //   } catch (error: any) {
// // //     console.error('Error in fetchSunChartData:', error);
...
// // //       });
// // //     } else {
// // //       res.status(500).json({ message: 'Failed to fetch Sun Chart data', error: error.message });
// // //     }
// // //   }
// // // };
...
// //   console.log('DerivedData for Sun Chart:', JSON.stringify(response.data, null, 2));
...
// //   const chartData = {
// //     surya_lagna: {
// //       degree: derivedData.surya_lagna?.degree || derivedData.surya_lagna?.longitude || '',
// //       longitude: derivedData.surya_lagna?.longitude || '',
// //       sign: derivedData.surya_lagna?.sign || '',
// //     },
// //     planetary_positions: planetaryPositions,
...
// //           ascendant_sign: ascendantSign,
// //           ascendant_degree: parseDMSToDecimal(derivedData.surya_lagna?.longitude),
// //           ascendant_degree_formatted: derivedData.surya_lagna?.longitude || null,
...
// // // Update Sun Chart
// // export const updateSunChart = async (
// //   user: User,
// //   chartType: any,
// //   apiEndpoint: string,
// //   existingChart: any
...
// // // Insert Sun Chart
// // export const insertSunChart = async (req: Request, res: Response): Promise<void> => {
// //   try {
// //     const { id } = req.params;
...
// //     });
// //     if (existingChart) {
// //       res.status(400).json({ message: 'Sun Chart already exists for user' });
// //       return;
// //     }
...
// //     res.status(200).json({
// //       message: 'Sun Chart inserted successfully',
// //       data: { derived_chart_id: derivedChartId, chart_type: 'SUN', data: derivedData },
// //     });
// //   } catch (error) {
// //     handleError(res, error, 'Failed to insert Sun Chart');
// //   }
// // };

// // // Fetch Sun Chart
// // export const fetchSunChart = async (req: Request, res: Response): Promise<void> => {
// //   try {
// //     const { id } = req.params;
...
// //       where: { user_id: user.user_id, chart_type_id: chartType.chart_type_id },
// //     });
// //     let message = 'Sun Chart fetched successfully';

// //     if (!existingChart) {
// //       const derivedData = await callAstroApi(apiEndpoint, buildApiPayload(user));
// //       const derivedChartId = await saveOrUpdateChartData(derivedData, chartType, user, apiEndpoint);
// //       existingChart = await DerivedChart.findOne({ where: { derived_chart_id: derivedChartId } });
// //       message = 'Sun Chart created and fetched successfully';
// //     } else if (hasUserDetailsChanged(user, existingChart.calculation_parameters)) {
// //       const derivedChartId = await updateSunChart(user, chartType, apiEndpoint, existingChart);
// //       existingChart = await DerivedChart.findOne({ where: { derived_chart_id: derivedChartId } });
// //       message = 'Sun Chart updated and fetched successfully';
// //     }

// //     if (!existingChart) throw new Error('Failed to retrieve Sun Chart');
...
// //     const responseData = {
// //       surya_lagna: (existingChart.chart_data as any).surya_lagna || {},
...
// //     });
// //   } catch (error) {
// //     handleError(res, error, 'Failed to fetch Sun Chart');
// //   }
// // };
// import { Request, Response } from 'express';
// import axios from 'axios';
...
//   if (cached) {
//     console.log('Returning Sun Chart from Redis cache.');
...
//     const cacheKey = `sunchart:${user.user_id}`;
...
//     const redisKey = `sun_chart_user_${updatedUser.user_id}`;
//     const cacheKey = `sunchart:${updatedUser.user_id}`;
...
//     ) {
//       console.log(`Serving Sun Chart for userId ${updatedUser.user_id} from Redis cache`);
//       res.status(200).json({
//         message: 'Sun Chart fetched successfully from cache',
...
//     await redisClient.setEx(redisKey, 3600, JSON.stringify({ chart_type: 'SUN', data: responseData }));
...
export const fetchSunChart = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!existingChart) {
      ...
      const derivedChartId = await updateSunChart(updatedUser, chartType, apiEndpoint, existingChart);
      existingChart = await DerivedChart.findOne({ where: { derived_chart_id: derivedChartId } });
      message = 'Sun Chart updated and fetched successfully';
    }
