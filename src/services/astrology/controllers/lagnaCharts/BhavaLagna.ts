...
//   if (cached) {
//     console.log('Returning Bhava Lagna chart from Redis cache.');
...
//   console.log('Calling Astro API and saving to Redis');
//   await redisClient.set(cacheKey, JSON.stringify(response.data), { EX: 360 });
...
//   const chartData = {
//     bhava_lagna: derivedData.bhava_lagna || {},
...
//       house_system: derivedData.metadata?.house_system || 'Bhava',
//     },
//   };
...
//           ascendant_sign: derivedData.bhava_lagna?.sign || '',
//           ascendant_degree: derivedData.bhava_lagna?.degrees || 0,
//           ascendant_degree_formatted: derivedData.bhava_lagna?.degrees ? `${derivedData.bhava_lagna.degrees}°` : undefined,
...
// export const updateBhavaLagnaChart = async (
//   req: Request,
//   res: Response,
//   user: User,
//   chartType: any,
...
//     const cacheKey = `bhavaLagnaChart:${updatedUser.user_id}`;
...
//     return derivedChartId;
//   } catch (error) {
//     handleError(res, error, 'Failed to update Bhava Lagna chart');
//     throw error;
//   }
// };

// export const insertBhavaLagnaChart = async (req: Request, res: Response): Promise<void> => {
//   try {
//     const { id } = req.params;
...
//     const chartType = await ChartType.findOne({ where: { chart_code: 'BHAVA' } });
//     if (!chartType) throw new Error('Chart type Bhava Lagna not found');
//     const apiEndpoint = chartType.getDataValue('api_endpoint');
//     if (!apiEndpoint) throw new Error('API endpoint not configured for Bhava Lagna');
...
//     if (existingChart) {
//       res.status(400).json({ message: 'Bhava Lagna chart already exists for user' });
//       return;
//     }

//     const payload = buildApiPayload(user);
//     const cacheKey = `bhavaLagnaChart:${user.user_id}`;
...
//     res.status(200).json({
//       message: 'Bhava Lagna chart inserted successfully',
//       data: { derived_chart_id: derivedChartId, chart_type: 'BHAVA', data: derivedData },
//     });
//   } catch (error) {
//     handleError(res, error, 'Failed to insert Bhava Lagna chart');
//   }
// };

// export const fetchBhavaLagnaChart = async (req: Request, res: Response): Promise<void> => {
//   try {
//     const { id } = req.params;
...
//     const redisKey = `bhava_lagna_chart_user_${user.user_id}`;
//     const cachedChart = await redisClient.get(redisKey);
//     if (cachedChart) {
//       console.log(`Serving Bhava Lagna chart for userId ${user.user_id} from Redis cache`);
//       res.status(200).json({
//         message: 'Bhava Lagna chart fetched successfully from cache',
...
//     let message = 'BHAVA chart fetched successfully';
//     let derivedChartId: string;

//     await redisClient.del(redisKey);

//     if (!existingChart) {
//       const derivedData = await callAstroApi(apiEndpoint, buildApiPayload(user), `bhavaLagnaChart:${user.user_id}`);
//       derivedChartId = await saveOrUpdateChartData(derivedData, chartType, user, apiEndpoint);
//       message = 'Bhava Lagna chart created and fetched successfully';
//     } else if (hasUserDetailsChanged(user, existingChart.calculation_parameters) || (req.body && Object.keys(req.body).length > 0)) {
//       derivedChartId = await updateBhavaLagnaChart(req, res, user, chartType, apiEndpoint, existingChart);
//       message = 'Bhava Lagna chart updated and fetched successfully';
//     } else {
...
//     if (!existingChart) throw new Error('Failed to retrieve Bhava Lagna chart');
...
//     await redisClient.setEx(redisKey, 360, JSON.stringify(responseData));
...
//     });
//   } catch (error) {
//     handleError(res, error, 'Failed to fetch Bhava Lagna chart');
//   }
// };
...
const transformResponse = (chart: any, planets: any[], user: any) => {
  ...
  return {
    ...
    bhava_lagna: {
      degrees: chartData.bhava_lagna?.degrees || '',
      sign: chartData.bhava_lagna?.sign || '',
    },
    notes: {
      ayanamsa: notes.ayanamsa || 'Lahiri',
      chart_type: notes.chart_type || 'Bhava Lagna',
      house_system: notes.house_system || 'Bhava',
    },
...
export const updateBhavaLagnaChart = async (
  ...
  try {
    ...
    if (!existingChartAfterUpdate) throw new Error('Failed to retrieve updated Bhava Lagna chart');
    ...
    res.status(201).json({
      message: 'Bhava Lagna chart updated successfully',
      data: responseData,
    });
...
export const insertBhavaLagnaChart = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!newChart) throw new Error('Failed to retrieve new Bhava Lagna chart');
...
export const fetchBhavaLagnaChart = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    let message = 'Bhava Lagna chart fetched successfully';
    let derivedChartId: string;
    let targetUser = user;
    ...
    if (!chart) throw new Error('Failed to retrieve Bhava Lagna chart');
