...
//     significators: chartData.significators || {},
...
//     significators: derivedData.significators || {},
//     metadata: {
//       ayanamsa: derivedData.metadata?.ayanamsa || 'KP',
//       chart_type: derivedData.metadata?.chart_type || `${chartType.chart_name} (${chartType.chart_code})`,
//       house_system: derivedData.metadata?.house_system || 'KP',
//     },
//   };
...
//           ascendant_sign: derivedData.lagna_sign || '',
//           ascendant_degree: derivedData.house_cusps?.[0]?.longitude || 0,
//           ascendant_degree_formatted: derivedData.house_cusps?.[0]?.longitude ? `${derivedData.house_cusps[0].longitude}°` : undefined,
...
// export const updateKPLagnaChart = async (
//   req: Request,
//   res: Response,
//   user: User,
//   chartType: any,
...
//     if (!existingChartAfterUpdate) throw new Error('Failed to retrieve updated KP Lagna chart');
...
//     res.status(201).json({
//       message: 'KP Lagna chart updated successfully',
//       data: responseData,
//     });

//     return derivedChartId;
//   } catch (error) {
//     handleError(res, error, 'Failed to update KP Lagna chart');
//     throw error;
//   }
// };

// export const insertKPLagnaChart = async (req: Request, res: Response): Promise<void> => {
//   try {
//     const { id } = req.params;
...
//     const chartType = await ChartType.findOne({ where: { chart_code: 'KP' } });
//     if (!chartType) throw new Error('Chart type KP Lagna not found');
//     const apiEndpoint = chartType.getDataValue('api_endpoint');
//     if (!apiEndpoint) throw new Error('API endpoint not configured for KP Lagna');
...
//     if (existingChart) {
//       res.status(400).json({ message: 'KP Lagna chart already exists for user' });
//       return;
//     }
...
//     if (!newChart) throw new Error('Failed to retrieve new KP Lagna chart');
...
//     res.status(201).json({
//       message: 'KP Lagna chart inserted successfully',
//       data: responseData,
//     });
//   } catch (error) {
//     handleError(res, error, 'Failed to insert KP Lagna chart');
//   }
// };

// export const fetchKPLagnaChart = async (req: Request, res: Response): Promise<void> => {
//   try {
//     const { id } = req.params;
...
//     let message = 'KP Lagna chart fetched successfully';
...
//       message = 'KP Lagna chart created and fetched successfully';
...
//       const derivedData = await callAstroApi(apiEndpoint, buildApiPayload(targetUser));
//       derivedChartId = await saveOrUpdateChartData(derivedData, chartType, targetUser, apiEndpoint, existingChart);
//       message = 'KP Lagna chart updated and fetched successfully';
//     } else {
//       // Use existing chart
...
//     if (!chart) throw new Error('Failed to retrieve KP Lagna chart');
...
//     });
//   } catch (error) {
//     handleError(res, error, 'Failed to fetch KP Lagna chart');
//   }
// };
import { Request, Response } from 'express';
import axios from 'axios';
...
export const updateKPLagnaChart = async (
  ...
  try {
    ...
    const redisKey = `kp_lagna_chart_user_${user.user_id}`;
    const cacheKey = `kplagnachart:${user.user_id}`;
    ...
    const responseData = transformResponse(existingChartAfterUpdate, planets, updatedUser);
    // Remove birth_details from final response to match original output
  ...
  try {
    ...
    if (cachedChart) {
      ...
      if (!hasUserDetailsChanged(user, cachedData.birth_details)) {
        console.log(`Serving KP Lagna Chart for userId ${user.user_id} from Redis cache`);
        // Remove birth_details from final response
        const { birth_details, ...responseWithoutBirthDetails } = cachedData;
        res.status(200).json({
          message: 'KP Lagna Chart fetched successfully from cache',
          data: responseWithoutBirthDetails,
        });
        return;
      }
